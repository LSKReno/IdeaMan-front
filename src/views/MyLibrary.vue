  ...
  <div>
    <Row :gutter="8">
      <Col :span="rightOpen ? 16 : 24">
        <input id="uploadbibfile" type="file" value style="visibility:hidden" @change="uploads" />
        <Table
          border
          stripe
          ref="selection"
      ...
      <Col v-if="rightOpen" :span="8">
        ...
        <Card class="box-in-detail" v-show="!modifyMode" dis-hover>
          ...
          <p style="margin-top: 10px;">
            {{
            authorsLine(nowData[currentIndex].authors) }}
          </p>
          <p style="margin-top: 10px;">
...
import { getLibrary as GetLibraryApi } from "../api/library";
import { addLibrary as AddLibraryApi } from "../api/library";
import { updateLibrary as UpdateLibraryApi } from "../api/library";
import { deleteLibrary as DeleteLibraryApi } from "../api/library";
...
export default {
...

        {
          ...
          render: (h, { row, column, index }) => {
            return `<div style='margin: 5px 0'>
                      <h4>
                        <a style='color:black' @click='() => {this.showDetail(index);}'>{{row.title}}</a>
                      </h4>
                      <p>authorsLine(row.authors)</p>
                      <Tag v-for='tag in row.tags'>{{tag}}</Tag>
                    </div>`;
          },
          renderHeader: (h) =>
            h(
  ...
  methods: {
    ...
    async deletelibrary() {
      ...
      try {
        let deleteRes = await DeleteLibraryApi({
          id: this.paper_id,
          deleted: 0,
        });
        if (deleteRes.code === 0) {
...
//       ),
//     ]),
//     h("p", [this.authorsLine(row.authors)]),
